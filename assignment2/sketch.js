...
function draw() {
	...
	for (let y = 0; y < height; y += 75) { // create a variable called y. loop the code below as long as y is less than height. increase y by 100 with each loop.
		for (let x = 0; x < width; x += 100) {// create a variable called x. loop the code below as long as x is less than width. increase x by 100 with each loop.
			...
			push(); // container start
			translate(x, y);
      rotation = map(y,500,height -200,100,1.7);
      rotate(rotation);
